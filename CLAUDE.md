...
- **시각 기준은 `DESIGN.md`다.** 공식 Panda 프리셋을 바탕으로 문서에 지정된
  값만 `panda.config.ts`에 등록한다. 색상을 소스나 대괄호 이스케이프에 직접
  쓰지 않는다.
- **라이트/다크 짝은 `ui.*` 시맨틱 토큰이 든다.** 새 UI는 역할 토큰을 쓰고,
  색을 바꿨으면 두 테마를 확인한다. `_dark`는 테마별 이미지 표시처럼 실제
  구조를 가르는 경우에만 컴포넌트에서 쓴다.
