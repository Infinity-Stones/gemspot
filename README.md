...
| `OSM_ROUTING_BASE_URL`                 | OSRM 도보 서버 주소 (선택)                 | 기본 공개 서버 |
...
생성 전에 시간이 이미 지났다는 질문이 반복되면 날짜와 시작·종료 시각을 함께
수정한다(예: **내일 오후 2시부터 4시까지**, **지금부터 두 시간**).
수정 답변은 이전 대화와 구분해 우선 적용하도록 모델에 전달하고, 500자 제한에서도
최신 답변을 보존한다. 시작 시각만으로 종료 시각을 확정할 수 없으면 다시 묻는다.
