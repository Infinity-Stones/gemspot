...
`/route`에서 “내일 오후 2시부터 4시까지 성수동에서 카페 들르면서 걷고 싶어”처럼
말하면 **저장한 장소** 중 방문할 곳을 골라 첫 결과를 바로 보여 준다.
시간이나 동네가 없으면 해당 조건만 되묻고, 이미 파악한 선호·필수 장소는 유지한다.
장소가 없거나 저장소를 읽지 못하면 그 이유를 안내한다. 예시 장소로 대체하지 않는다.

첫 결과에는 날짜·시간·선호 요약, 동네의 대표 출발점, 방문 번호 지도,
장소별 도착·체류 시간과 총 도보 시간이 함께 나온다. 사용자가 원할 때만 다음을 수정한다.

- **조건 수정 → 다시 제안**: 날짜·시간·동네·선호·필수 장소를 고친다. 고친 조건은
...
보행 경로는 `TMAP_APP_KEY`를 설정하고 서버를 재시작해야 실제로 조회한다.
성공 구간은 지도 실선, 조회 실패·키 누락 구간은 점선과 “추정” 표시로 구분한다.
모든 구간이 추정이면 실제 보행 경로 연동 검증이 끝난 것으로 보지 않는다.
네이버 SDK는 `src/shared/naverMap.ts`의 클라이언트 ID와 콘솔의 Web 서비스 URL 등록,
Dynamic Map 사용 설정이 맞아야 한다. 인증·로딩 실패 때는 지도 재시도를 안내하고
방문 목록과 동선 편집은 유지한다.

연동 확인 시에는 저장한 장소가 있는 브라우저로 `/route`를 열어 자연어 요청 →
되묻기 → 첫 결과 → 조건 수정 → 장소 빼기·복원·순서 변경을 확인한다.
TMAP이 설정된 환경에서는 실제 길을 따르는 선과 `tmap` 구간의 거리·시간을 확인하고,
키 누락이나 API 실패 환경에서는 추정 표시와 오류 후 입력 보존을 확인한다.
