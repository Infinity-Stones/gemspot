...
describe('toSpotCandidates', () => {
  ...
  it('id가 이미지와 순서로 결정된다 — 같은 입력이면 같은 id다', () => {
    ...
    const once = toSpotCandidates([{ name: 'a', addressLine: '' }], 'img-9');
    const twice = toSpotCandidates([{ name: 'a', addressLine: '' }], 'img-9');
  ...
  it('출처를 ocr로 남긴다 — 직접 입력과 구분해야 한다', () => {
    expect(
      toSpotCandidates([{ name: 'a', addressLine: '' }], 'i')[0]?.origin,
    ).toBe('ocr');
  });
});
...
describe('partitionByRoadAddress', () => {
  it('성공 건과 실패 건을 가른다', () => {
    const { resolved, unresolved } = partitionByRoadAddress(
      toSpotCandidates(
        [
          { name: '피롤츠', addressLine: '서울 용산구 한강대로 56-1' },
          { name: '에그앤플라워', addressLine: '용산동2가' },
          { name: '이름만', addressLine: '' },
        ],
        'img-1',
      ),
    );
  ...
  it('버리지 않는다 — 둘을 합치면 넣은 수다', () => {
    const candidates = toSpotCandidates(
      [
        { name: 'a', addressLine: '화랑로 608' },
        { name: 'b', addressLine: '용산구' },
      ],
      'img-1',
    );
  ...
  it('입력 순서를 지킨다 — 앨범에서 고른 순서가 목록 순서다', () => {
    const { unresolved } = partitionByRoadAddress(
      toSpotCandidates(
        [
          { name: 'first', addressLine: '용산구' },
          { name: 'second', addressLine: '한강동' },
        ],
        'img-1',
      ),
    );
