...
import { matchRequiredSpots, planFromRequest, planRoute, reschedule } from './planner';
...
const foundGeocode: GeocodeFn = () =>
  Promise.resolve({
    ...
    data: {
      totalCount: 1,
      hits: [{ coord: START, roadAddress: '서울특별시 성동구 성수동1가', jibunAddress: '', region: null }],
    },
  });
...
describe('planRoute — 제안하지 않는 경우(T47)', () => {
  it('시간대가 없으면 needs_clarification이고 이후 단계는 부르지 않는다', async () => {
    ...
    const outcome = await planRoute({
      ...
      now: NOW,
      spots: ALL_SPOTS,
      deps: { generate: stubGenerate([{ ...SPEC_INTERPRETATION, window: null }]), geocode },
    });
    expect(outcome).toMatchObject({
      kind: 'failed',
      failure: { kind: 'needs_clarification', missing: ['window'], question: '몇 시부터 몇 시까지요?' },
    });
  ...
  it('동네가 좌표로 안 바뀌면 area_not_found', async () => {
    const outcome = await planRoute({
      ...
      spots: ALL_SPOTS,
      deps: {
        generate: stubGenerate([{ ...SPEC_INTERPRETATION, areaName: '없는동' }]),
        geocode: () => Promise.resolve({ ok: true, data: { totalCount: 0, hits: [] } }),
      },
    });
    expect(outcome).toMatchObject({ kind: 'failed', failure: { kind: 'area_not_found', areaName: '없는동' } });
  });
  ...
  it('그 시간대에 갈 만한 스팟이 없으면 no_candidates, LLM 순서 제안은 부르지 않는다', async () => {
    ...
    expect(outcome).toMatchObject({ kind: 'failed', failure: { kind: 'no_candidates', areaName: '성수동' } });
  ...
  it('LLM 키가 없으면 service_unavailable(llm)', async () => {
    const outcome = await planRoute({
      ...
      now: NOW,
      spots: ALL_SPOTS,
      deps: { generate: () => Promise.resolve({ ok: false, error: { kind: 'no_api_key' } }) },
    });
    expect(outcome).toMatchObject({ kind: 'failed', failure: { kind: 'service_unavailable', service: 'llm' } });
  });
});
...
describe('planFromRequest — 외부 서비스 없이', () => {
  ...
  it('필수 스팟은 반경 · 시간대 밖이어도 여정에 들어간다', async () => {
    ...
    expect(outcome.itinerary.stops.map((s) => s.candidate.id)).toContain(FOOD_D.id);
  });
});
...
describe('planFromRequest — 종료 시각 초과(T42)', () => {
  ...
  const tight = {
    ...REQUEST_14_16,
    window: { start: '2026-09-12T14:00:00+09:00', end: '2026-09-12T14:40:00+09:00' },
  };
  ...
  it('LLM에 한 번만 다시 묻고, 그래도 넘으면 비필수를 뒤에서 뺀다', async () => {
    ...
    const outcome = await planFromRequest({
      ...
      deps: {
        ...
        generate: stubGenerate([{ order: ['a', 'b', 'c', 'e'], reasons: [] }], calls),
    ...
    expect(outcome.itinerary.stops.map((s) => s.candidate.id)).toEqual([SHOP_A.id]);
    expect(outcome.itinerary.dropped.filter((d) => d.reason === 'over_time').map((d) => d.candidate.id)).toEqual(
      ['e', 'c', 'b'],
    );
    expect(outcome.itinerary.overBySeconds).toBe(0);
  });
    ...
    const outcome = await planFromRequest({
      ...
      deps: {
        generate: (input) => {
          n += 1;
          return n === 1
            ? Promise.resolve({ ok: true, data: { order: ['a', 'b', 'c', 'e'], reasons: [{ id: 'a', reason: 'r' }] } })
...
describe('reschedule (T45)', () => {
  it('바뀌지 않은 구간은 재실측하지 않고, 순서가 바뀐 정거장의 이유는 지운다', async () => {
    ...
    const first = await planFromRequest({
      request: REQUEST_14_16,
      spots: [SHOP_A, CAFE_B, BOOK_C],
      deps: { generate: stubGenerate([SPEC_PROPOSAL]), route: stubRoute(SPEC_LEG_SECONDS, calls) },
    });
    ...
    const next = await reschedule({
      ...
      previousReasons: new Map(first.itinerary.stops.map((s) => [s.candidate.id, s.reason ?? ''])),
...
describe('matchRequiredSpots', () => {
  it('공백 · 대소문자 무시, 부분 일치 허용, 못 맞추면 unmatched', () => {
    const { ids, unmatched } = matchRequiredSpots(['카페b', '서점', '없는곳'], ALL_SPOTS);
