...
describe('measureLegs', () => {
  it('OSM 경로의 출처와 도로 경로선을 보존한다', async () => {
    const path = [
      START,
      { latitude: 37.543, longitude: 127.0558 },
      SHOP_A.coord,
    ];
    const legs = await measureLegs({
      ...
      route: () =>
        Promise.resolve({
          ok: true,
          source: 'osm',
          data: { distanceM: 441.3, durationS: 355.2, path },
        }),
    });
  ...
  it('빈 순서는 빈 배열', async () => {
    expect(
      await measureLegs({ start: START, order: [], route: failingRoute }),
    ).toEqual([]);
  });
});
