...
const found: GeocodeOutcome = { ok: true, data: { totalCount: 1, hits: [HIT] } };
const notFound: GeocodeOutcome = { ok: true, data: { totalCount: 0, hits: [] } };
const noKey: GeocodeOutcome = { ok: false, error: { kind: 'no_api_key' } };

const SAVED_ROW = {
  id: '5f6d1c2e-0000-4000-8000-000000000002',
  name: '피롤츠 커피하우스',
  road_address: HIT.roadAddress,
  jibun_address: HIT.jibunAddress,
  latitude: 37.5299,
  longitude: 126.9648,
  sido: '서울특별시',
  sigugun: '용산구',
...
const inserting = (result: InsertRowResult) => vi.fn(() => Promise.resolve(result));

const input = { name: '피롤츠 커피하우스', address: '서울 용산구 한강대로 56-1', category: 'cafe' as const, origin: 'manual' as const };

describe('locateAddress', () => {
  it('좌표가 있으면 ready, 0건이면 not_found, 키 없음은 unavailable', async () => {
    await expect(locateAddress('x', { geocode: () => Promise.resolve(found) })).resolves.toMatchObject({
      kind: 'ready',
      location: { coordinates: HIT.coord, roadAddress: HIT.roadAddress },
    });
    await expect(locateAddress('x', { geocode: () => Promise.resolve(notFound) })).resolves.toEqual({ kind: 'not_found' });
    await expect(locateAddress('x', { geocode: () => Promise.resolve(noKey) })).resolves.toEqual({
      kind: 'unavailable',
      reason: 'no_api_key',
    });
  });
...
describe('saveSpot — 좌표가 나오지 않으면 저장하지 않는다(T22)', () => {
  it('주소가 좌표로 바뀌지 않으면 insert를 부르지 않는다', async () => {
    const insert = inserting({ ok: true, row: SAVED_ROW });
    const outcome = await saveSpot(input, { geocode: () => Promise.resolve(notFound), insert });
    expect(outcome).toEqual({ kind: 'failed', failure: { kind: 'address_not_found' } });
    expect(insert).not.toHaveBeenCalled();
  });

  it('Geocoding이 불가하면(키 없음 · 네트워크) 저장하지 않는다', async () => {
    const insert = inserting({ ok: true, row: SAVED_ROW });
    const outcome = await saveSpot(input, { geocode: () => Promise.resolve(noKey), insert });
    expect(outcome).toEqual({ kind: 'failed', failure: { kind: 'geocoding_unavailable' } });
    expect(insert).not.toHaveBeenCalled();
  });

  it('빈 이름 · 빈 주소는 Geocoding도 부르지 않는다', async () => {
    const geocode = vi.fn(() => Promise.resolve(found));
    await expect(saveSpot({ ...input, name: '  ' }, { geocode })).resolves.toEqual({
      kind: 'failed',
      failure: { kind: 'invalid_input', field: 'name' },
    });
    await expect(saveSpot({ ...input, address: '' }, { geocode })).resolves.toEqual({
      kind: 'failed',
  ...
  it('좌표가 있으면 Geocoding 결과의 주소 · 지역으로 행을 만들어 저장하고, 저장된 행을 돌려준다', async () => {
    const insert = inserting({ ok: true, row: SAVED_ROW });
    const outcome = await saveSpot(input, { geocode: () => Promise.resolve(found), insert });
    ...
    expect(outcome).toMatchObject({ kind: 'saved', spot: { id: SAVED_ROW.id, name: '피롤츠 커피하우스' } });
  });

  it('저장소가 없으면 store_unconfigured — 시드에 끼워 넣지 않는다', async () => {
    const outcome = await saveSpot(input, {
      geocode: () => Promise.resolve(found),
      insert: inserting({ ok: false, error: { kind: 'unconfigured' } }),
    });
    expect(outcome).toEqual({ kind: 'failed', failure: { kind: 'store_unconfigured' } });
  });

  it('DB 오류는 store_error로 메시지와 함께', async () => {
    const outcome = await saveSpot(input, {
      geocode: () => Promise.resolve(found),
      insert: inserting({ ok: false, error: { kind: 'query', message: 'check constraint' } }),
    });
    expect(outcome).toEqual({ kind: 'failed', failure: { kind: 'store_error', message: 'check constraint' } });
  });
});
