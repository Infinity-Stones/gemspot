 ...
 */
export async function pinSpotAction(
  previous: PinFormState,
  formData: FormData,
  ...
  if (formData.has('pickPlace') && picked === null) {
    return {
      status: 'invalid',
      field: 'name',
      draft: typed,
      message: '검색 결과에서 가게를 다시 선택해 주세요.',
    };
  }
  ...
  if (missing !== null) {
    return {
      status: 'invalid',
      field: missing,
      message: messageForMissing(missing),
      draft,
    };
  }
  ...
  if (intent === 'search_place') {
    ...
    if (found.kind === 'results')
      return { status: 'place_searched', draft, places: found.places };
    return {
      status: 'failed',
      draft,
      failure:
        found.kind === 'not_found'
          ...
          : found.reason === 'no_api_key'
            ? { kind: 'place_search_unconfigured' }
            : { kind: 'place_search_unavailable' },
    };
  }
  ...
  if (intent === 'search') {
    ...
    if (searched.kind === 'results') {
      ...
      const only =
        searched.candidates.length === 1 ? searched.candidates[0] : undefined;
      if (only !== undefined)
        return {
          status: 'located',
          draft: { ...draft, address: addressOf(only) },
          location: only,
        };
  ...
  if (intent === 'locate') {
    ...
    if (located.kind === 'ready')
      return { status: 'located', draft, location: located.location };
    return {
      status: 'failed',
      draft,
      failure:
        located.kind === 'not_found'
          ? { kind: 'address_not_found' }
          : { kind: 'geocoding_unavailable' },
    };
  }

  // 화면에서 숨기는 것만으로는 수정된 주소의 저장을 막을 수 없다.
  // 서버에서도 마지막으로 미리 본 주소인지 확인하고 좌표는 다시 조회한다.
  if (
    previous.status !== 'located' ||
    previous.draft.address !== draft.address
  ) {
    return {
      status: 'invalid',
      field: 'address',
      draft,
      message: '저장할 주소의 위치를 먼저 확인해 주세요.',
    };
  }
  ...
  if (outcome.kind === 'failed')
    return { status: 'failed', draft, failure: outcome.failure };
 ...
 */
function readPickedPlace(
  previous: PinFormState,
  formData: FormData,
): FoundPlace | null {
  const raw = readText(formData, 'pickPlace');
  if (!/^\d+$/.test(raw) || previous.status !== 'place_searched') return null;
...
function addressOf(location: {
  ...
  return location.roadAddress.length > 0
...
function readDraft(formData: FormData): PinDraft {
  ...
  const category: SpotCategory = isSpotCategory(rawCategory)
    ? rawCategory
    : 'other';
  return {
