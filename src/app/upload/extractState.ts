...
/**
 * 만든 후보와 그 원본 업로드 이미지를 결과 화면으로 넘기는 자리.
 *
 * 결과가 다른 라우트에 있고(D03 · #17), 고른 파일은 `File` 객체라 라우트를
 * 넘지 못한다. 확정 전 후보를 저장소에 넣을 수도 없다 — 사용자가 확정해야
 * 스팟이 된다는 것이 STEP 3의 전제다.
 *
 * 그래서 브라우저의 `sessionStorage`를 거친다. 서버가 돌려준 후보에
 * 클라이언트가 들고 있던 업로드 이미지 id·미리보기 URL·파일명을 별도 속성으로
 * 붙여 담는다. 탭 안에서만 살고 닫으면 사라지는 값인데, 확정 전 후보와 이미지의
 * 수명이 정확히 그렇다. 서버로 갈 이유도 없다 — 사용자가 저장을 고르기 전까지
 * 이 데이터는 아무 데도 남지 않아야 한다.
 */
