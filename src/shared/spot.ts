 ...
 *
 * **목록은 고정이다 — 사용자가 카테고리를 만들지 못한다**(D07 #27의 결정).
 * 열어 두려면 시간대와 기본 체류 시간을 사용자에게 받아야 하는데, 그 둘이
 * 동선 엔진이 후보를 거르고 시간을 배치하는 유일한 근거다. 값 없이 만들면
 * 그 카테고리의 스팟은 아무 시간대에나 끼워지고, 받자면 "노래방 몇 시부터
 * 몇 시까지"를 스팟마다 묻게 된다.
 *
 * 이 목록은 D11의 카테고리별 적합 시간대 표와 기본 체류 시간의 행 집합과
 * 같다 — **하나를 늘리면 저쪽에 행이 하나 없는 상태가 된다.** 그래서 값이
 * 늘거나 줄면 그 표를 함께 고쳐야 하고, `spots.category`의 DB 체크 제약도
 * 같은 집합이라 마이그레이션이 함께 간다.
 */
export const SPOT_CATEGORIES = [
  // 끼니를 먹는 곳. 점심·저녁 시간대에만 후보다.
  'meal',
  // 카페 · 디저트.
  'cafe',
  // 영화관. 체류가 길어 짧은 창에는 후보로 서지 못한다.
  'movie',
  // 오락 — 노래방 · 게임장 · 방탈출처럼 놀러 가는 곳.
  'amusement',
  // 스포츠 — 하는 것(클라이밍 · 볼링)과 보는 것(경기장)을 함께 둔다. 둘을
  // 가르려면 시간대가 아니라 "직접 하는가"를 물어야 하는데 그 축이 계약에 없다.
  'sports',
