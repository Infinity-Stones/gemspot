...
/**
 * LLM 어댑터 — D12(#44)의 결정(Gemini, `@google/genai`).
 *
 ...
 * 규칙을 알 수 없고, 알아서도 안 된다). 제공자를 바꾸면 이 파일만 바뀐다.
 *
...
export type GenerateJsonResult =
  ...
  | { readonly ok: false; readonly error: HttpFailure | { readonly kind: 'no_api_key' } };

export type GenerateJson = (input: GenerateJsonInput) => Promise<GenerateJsonResult>;

/** 구조화 출력은 수 초가 걸린다. GET 기본값(5초)보다 넉넉해야 한다. */
const DEFAULT_TIMEOUT_MS = 15_000;

/** SDK 호출 한 번의 최소 면. 테스트에서 이것만 갈아 끼운다. */
export type GeminiCaller = (params: {
...
function sdkCaller(apiKey: string): GeminiCaller {
  const ai = new GoogleGenAI({ apiKey });
  return async ({ model, system, user, schema, signal }) => {
...
export interface CreateGenerateJsonOptions {
  ...
  readonly model?: string;
  readonly caller?: GeminiCaller;
}

/**
 * `generateJson`을 만든다. 기본은 env의 키 · 모델과 실제 SDK. 키가 없으면 매
 * 호출이 `no_api_key`로 끝난다 — 만들 때 던지지 않는 이유는 키 없이도 앱이
 * 떠야 하기 때문이다.
 */
export function createGenerateJson(options: CreateGenerateJsonOptions = {}): GenerateJson {
  ...
  return async ({ system, user, schema, timeoutMs = DEFAULT_TIMEOUT_MS }) => {
    ...
    try {
      text = await caller({ model, system, user, schema, signal: AbortSignal.timeout(timeoutMs) });
    } catch (cause) {
      const isTimeout =
    ...
    if (text === undefined || text.length === 0) {
      return { ok: false, error: { kind: 'parse', message: '응답 본문이 비어 있습니다' } };
    }
    try {
...
export function generateJson(input: GenerateJsonInput): Promise<GenerateJsonResult> {
