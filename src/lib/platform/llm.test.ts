...
import type { JsonCaller } from './llm';
import { createGenerateJson } from './llm';

const input = { system: 's', user: 'u', schema: { type: 'object' } };

describe('createGenerateJson', () => {
  it('API가 준 텍스트를 JSON으로 파싱해 돌려준다', async () => {
    const caller: JsonCaller = vi.fn(() => Promise.resolve('{"a":1}'));
    ...
    expect(caller).toHaveBeenCalledWith(
      expect.objectContaining({ model: 'm', system: 's', user: 'u' }),
    );
  });
  ...
  it('빈 응답 · JSON 아닌 응답은 parse', async () => {
    const empty = createGenerateJson({
      apiKey: 'k',
      caller: () => Promise.resolve(undefined),
    });
    const broken = createGenerateJson({
      apiKey: 'k',
      caller: () => Promise.resolve('not json'),
    });
  ...
  it('API가 던지면 timeout과 network를 가른다', async () => {
    const timeoutErr = Object.assign(new Error('t'), { name: 'TimeoutError' });
    const timedOut = createGenerateJson({
      apiKey: 'k',
      caller: () => Promise.reject(timeoutErr),
    });
    const failed = createGenerateJson({
      apiKey: 'k',
